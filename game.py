...
class Play:
    ...
    def __init__(self, hero, enemy, labyrinth, view):
        ...
        self.elements = {
            "MACGYVER" : [st.MACGYVER, pygame.image.load(st.MACGYVER_IMAGE), self.hero.position],
            "ENEMY" : [st.ENEMY, pygame.image.load(st.ENEMY_IMAGE), self.enemy.position],
            "ROADS" : [st.ROADS, pygame.image.load(st.ROAD_IMAGE), self.laby.roads],
            "WALLS" : [st.WALLS, pygame.image.load(st.WALL_IMAGE), self.laby.walls]
        }
        ...
        self.laby.set_items()
        self.laby.add_roads(self.hero.position[0])
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP and self.play is True:
                        self._check(self.hero.get_pos_x, self.hero.get_pos_y-1)

                    if event.key == pygame.K_DOWN and self.play is True:
                        self._check(self.hero.get_pos_x, self.hero.get_pos_y+1)

                    if event.key == pygame.K_RIGHT and self.play is True:
                        self._check(self.hero.get_pos_x+1, self.hero.get_pos_y)
    ...
    def _check(self, pos_x, pos_y):
        ...
        if is_inside is True:
            if (pos_x, pos_y) in self.laby.items_list:
                self._moving(pos_x, pos_y, self.hero.get_pos_x, self.hero.get_pos_y)
            ...
            elif (pos_x, pos_y) in self.enemy.position:
                self._moving(pos_x, pos_y, self.hero.get_pos_x, self.hero.get_pos_y)
                self.hero.move((pos_x, pos_y))
                self.endgame()
    ...
    def _moving(self, new_pos_x, new_pos_y, pos_x, pos_y):
        """ Update positions. """
        pos_hero = ((new_pos_x * st.SPRITE_SIZE), ((new_pos_y * st.SPRITE_SIZE)))
